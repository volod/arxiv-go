...
| W10 CATIA text recipes | The two PowerShell snippets under [Collect CATIA text for a search index](manual-windows.md#collect-catia-text-for-a-search-index), on an archive split with `--catia-text` | One `.catia.md` per sidecar and one assembly index; UTF-8 without BOM, `archive:` first, component lists complete, no stray `strings:` line. Written on Linux against the tested Bash equivalents and never executed; `utf8NoBOM` needs PowerShell 7+ |
