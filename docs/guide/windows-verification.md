...
| W10 CATIA text recipes | The two PowerShell snippets under [Collect CATIA text for a search index](manual-windows.md#collect-catia-text-for-a-search-index), on an archive split with `--catia-text` | One `.catia.md` per sidecar, byte-identical to it (`arxgo-text:` first, `archive:` second, the native root quoted as `"D:\\archive"`), and one assembly index; UTF-8 without BOM, component lists complete, no stray `strings:` line. Written on Linux against the tested Bash equivalents and never executed; `utf8NoBOM` needs PowerShell 7+ |
