...
| W7 Stage-1 and stage-4 proofs | `go test -tags integration ./test/integration/...` on NTFS (`TestArchiveSplitRestoreRoundTrip`, `TestCatiaSplitRestoreRoundTrip`), then with `ARXGO_TEST_VIDEO_PARENT` naming a directory on the second volume (both mirror roots of the stage-4 proof go there) | Manifests equal after split, kill, resume and restore of both payloads; exit codes as specified |
...
| W8 Release bundle | The PowerShell download, checksum and extract blocks of the README quick start and of [Download a release bundle](manual-windows.md#download-a-release-bundle) in Windows PowerShell 5.1 and PowerShell 7, against a published release; a ZIP downloaded with a browser; `arxgo.exe split --image start` on a generated video | Both checksum checks pass and a changed byte in the ZIP or in `arxgo.exe` throws; `Unblock-File` clears a browser download; previews produced with the bundled `ffmpeg.exe`; no `.env` in the bundle. The blocks were written on Linux against the tested Bash equivalents and never executed |
| W8a Source build | The steps of [Build from source](manual-windows.md#build-from-source) on a host without Go: `winget` installs, the `go build` command, the pinned FFmpeg download | `bin\arxgo.exe version` prints the `VERSION` file; `ffmpeg.exe` and `ffprobe.exe` match `packaging\ffmpeg.lock` and a changed pin throws; with the same Go version, the executable equals the one `make build-all` cross-builds on Linux |
| W9 Cloud publishing | Token cache and session state after a publish | Files readable only by the current user (ACL) |
| W10 CATIA text | The copy snippet under [Collect CATIA text for a search index](manual-windows.md#collect-catia-text-for-a-search-index) and `arxgo.exe catia-index` (default output, `--out` with a case-variant spelling of `D:\archive\arxgo-catia.csv` and of an owned `.md`; `--strings` exits 2), and the snippet under [Move CATIA files to a CATIA archive](manual-windows.md#move-catia-files-to-a-catia-archive) that deletes owned sidecars holding `strings:` before a rerun split, on an archive split with `--catia-text`, on a drive-letter and a UNC archive root | One `.catia.md` per sidecar, byte-identical to it (`arxgo-text:` first, `archive:` second, the native root quoted as `"D:\\archive"`); the index has one section per `moved` row with slash `rel_path` headings, a byte-identical rerun, case-variant reserved and owned `--out` paths exit 2, `scan` does not register `arxgo-catia-text.md`; UTF-8 without BOM. The deleting snippet removes only sidecars whose first line is `arxgo-text: ` and that hold a `strings:` line, and the rerun split rewrites them with `notes:`. Both snippets were written on Linux against the tested Bash equivalents and never executed; `utf8NoBOM` needs PowerShell 7+ |
...
| `AUD-record-source-location-in-metadata-1` | `archive:` of descriptions and sidecars written with a drive-letter, UNC and `\\?\` long root: native spelling, quoting of backslashes, and the sidecar `description:` slash path | W10 |
| `AUD-implement-catia-text-index-1` | `catia-index` on a drive-letter and UNC root: case-folded reserved and owned `--out` refusal, atomic replace of an output open in another program, `history_at` and slash headings | W10 |
