...
| W2 CLI and environment | `make setup` under Git Bash; `ARXGO_EXCLUDE` with `;` separators; `.env` next to `arxgo.exe`; case-variant and `\\?\` long roots; `--exclude 'cache\*'` | Setup completes; list split on `;`; `.env` read; case variants of one root rejected as the same directory; backslash glob exits 2 |
...
| W6 Tool discovery | `ffprobe.exe` next to `arxgo.exe` and on `PATH`; fake `.bat` tools that fail `-version` | Next-to-executable wins; failing tool treated as missing; exit 3 prints the `windows/amd64` link |
