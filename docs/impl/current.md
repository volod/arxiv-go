...
| [Crash safety](current/crash-safety.md) | Filesystem primitives; run lock, `.arxgo/` layout, checkpoints, run log, progress, report, WAL and recovery, disk-space preflight | Shipped; session used by scan, split and restore |
...
| [Video restore](current/video-restore.md) | Restore videos with directory, conflict, stub and registry policies; crash recovery | Available; stage-1 checkpoint and proof remain |
