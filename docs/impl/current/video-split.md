...
With `--verify hash`, split records the SHA-256 on the same-device rename path too: it reads the
file once before the rename and writes the `verified` WAL record the payload registry is rebuilt
from, so `arxgo-videos.csv` and the description carry the same hash. Before this the column was
empty on every renamed row while the description held a hash. A rename moves no bytes, so nothing
is compared; restore keeps no hash column and skips the read
