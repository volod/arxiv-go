...
- Execution path: `make ffmpeg`, live media and archive tests and `make test-integration` with the
  pinned tools, `make dist` and bundle inspection, real-media round trip on the arhive copy.
...
| Real media with 9.0.1 | arhive-copy round trip (0033 declared run) with the pinned tools beside `arxgo` | pass: split 40 videos, 80 previews, 0 failed; rerun exit 0; restore exit 0; manifests and SHA-256 identical |
