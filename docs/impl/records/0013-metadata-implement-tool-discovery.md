...
- `AUD-implement-tool-discovery-1`: nonblocking, Windows only. `exec.LookPath` on
  ...
  test) or a copied real `ffprobe.exe` next to a failing one. Next check: portable fake tools are
  added by plan task `add-portable-fake-tools`; the Windows run itself is step W6 of the deferred
