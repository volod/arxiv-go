...
- Execution path: Unit tests of selection and arguments; live test on a generated file whose first
  audio stream has no decoder; arhive-copy run on the iPhone `apac` files.
...
| Decodable-first files unchanged | live sample tests; arhive-copy round trip | pass: 80 previews, no audio warning |
