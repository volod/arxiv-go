...
- Amendments:
  ...
  3. Declared run on real media, requested by the operator during the review: "Please scan
     /media/vola/vola-2tp1/_Drone/ arhive and check if we support all the required to handle such
     arhive video formats", after desktop players reported missing `meta/x-gst-fourcc-fdsc` and
...
Read-only survey of `/media/vola/vola-2tp1/_Drone` (473 GiB, 24,738 files; 627 video files,
287 GiB) with ffprobe: containers MP4 406, MOV 145, MTS 16, GoPro LRV 9, MPEG-1 elementary `.mpg`
...
rotation, data tracks; 3.9 GiB) was copied with its directory structure (Cyrillic names) into the
...
otherwise after it; 14 files fall back to ffprobe as specified (WhatsApp exports with a box past
the end of file, downloads without a readable box structure).
...
- `AUD-review-stage-2-previews-1`: nonblocking. FFmpeg 6.1.1 cannot decode Apple positional audio
  ...
  every run. On the drone archive both `apac` files carry AAC first and no MP2 source has audio.
...
human); next eligible `research-cloud-target-apis`. Temporary repro tests, the drone copy and the
