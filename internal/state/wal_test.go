...
// The WAL contract lists the fields a step after begin carries; a zero mtime is not one of them,
// and encoding/json ignores omitempty on a time.Time.
func TestRecordOmitsZeroMtime(t *testing.T) {
	later, err := json.Marshal(Record{V: WALVersion, TxID: "t-000001", Seq: 2, Step: StepPlaced, TS: time.Unix(0, 0).UTC()})
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Contains(later, []byte("mtime")) {
		t.Fatalf("later step carries mtime: %s", later)
	}
	at := time.Date(2026, 9, 15, 12, 0, 0, 123456789, time.UTC)
	begin, err := json.Marshal(Record{V: WALVersion, TxID: "t-000001", Seq: 1, Step: StepBegin, TS: at, Mtime: at})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(begin, []byte(`"mtime":"2026-09-15T12:00:00.123456789Z"`)) {
		t.Fatalf("begin lost its mtime precision: %s", begin)
	}
	var back Record
	if err := json.Unmarshal(begin, &back); err != nil || !back.Mtime.Equal(at) {
		t.Fatalf("round trip = %v, %v", back.Mtime, err)
	}
	// The WAL encoder keeps <, > and & literal; the marshaler must not start escaping them.
	raw, err := encodeRecord(Record{V: WALVersion, TxID: "t-000001", Seq: 3, Step: StepBegin,
		TS: at, Mtime: at, RelPath: "a&b/<c>.mp4"})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(raw, []byte(`"rel_path":"a&b/<c>.mp4"`)) {
		t.Fatalf("WAL escaped HTML characters in a path: %s", raw)
	}
}
