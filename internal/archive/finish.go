...
// names (videos_* for video, catia_* for CATIA). A scan has no payload and moves nothing, so it
// reports none of them rather than three zeroed video counters.
func (s *Session) payloadFinishAttrs(c state.Counters) []any {
	if s.payload == nil {
		return nil
	}
	if s.payload.kind == PayloadVideo {
