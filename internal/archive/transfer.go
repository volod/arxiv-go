...
// the SHA-256 recorded for the file with --verify hash, on either path.
func placePayload(ctx context.Context, s *Session, w *state.WAL, rec state.Record, mode string, verify fsops.VerifyMode, overwrite bool, stage stageCopyFunc) (string, error) {
	if mode == state.TransferRename && !overwrite {
		...
		sum, err := hashRenamed(w, rec, verify)
		if err != nil {
			return "", err
		}
		...
		return sum, hitCrash(s.cfg.Crash, "fs:place")
	}
	if stage == nil {
...
// hashRenamed records the SHA-256 of a file about to be renamed into the mirror when --verify hash
// asked for one. A rename moves the bytes without reading them, so there is nothing to compare; the
// hash is recorded so the payload registry, which is rebuilt from the WAL, carries the same value
// the description does. It is written before the rename, so a crash either aborts the whole
// transaction or rolls it forward with the hash already durable.
func hashRenamed(w *state.WAL, rec state.Record, verify fsops.VerifyMode) (string, error) {
	// Restore keeps no hash column, so only split pays for the read.
	if verify != fsops.VerifyHash || rec.Op != opSplit {
		return "", nil
	}
	sum, err := hashFile(rec.Src)
	if err != nil {
		return "", err
	}
	text := hex.EncodeToString(sum[:])
	if _, err := w.Append(rec.TxID, state.StepVerified, state.Record{SHA256: text}); err != nil {
		return "", err
	}
	return text, nil
}
