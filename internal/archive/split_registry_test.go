...
// --verify hash must fill the registry sha256 column on the rename path too: the description
// already carried a hash there, so the two outputs of one run disagreed.
func TestSplitRecordsHashOfRenamedPayload(t *testing.T) {
	r, src, dst := splitFixture(t)
	cfg, c := splitConfig(r, "auto")
	if res := runSplit(t, cfg, c); res.Status != StatusCompleted {
		t.Fatalf("split = %+v", res)
	}
	sum := sha256.Sum256(mustRead(t, dst))
	want := hex.EncodeToString(sum[:])
	rows, err := report.LoadVideoFile(filepath.Join(r.archive, scanner.VideoRegistryName))
	if err != nil || len(rows) != 1 {
		t.Fatalf("registry rows = %d: %v", len(rows), err)
	}
	if rows[0].Transfer != "rename" {
		t.Fatalf("transfer = %q, want rename", rows[0].Transfer)
	}
	if rows[0].SHA256 != want {
		t.Fatalf("registry sha256 = %q, want %q", rows[0].SHA256, want)
	}
	if got := string(mustRead(t, src+".md")); !strings.Contains(got, "sha256: "+want+"\n") {
		t.Fatalf("description does not carry the same hash:\n%s", got)
	}
}

// Without --verify hash a rename reads nothing, so no hash is recorded anywhere.
func TestSplitWithoutHashVerifyRecordsNoHash(t *testing.T) {
	r, src, _ := splitFixture(t)
	cfg, c := splitConfig(r, "auto")
	c.Verify = fsops.VerifySize
	c.Descriptions = NewMarkdownDescription(DescriptionConfig{
		Archive: r.archive, Mirror: r.video, Registry: c.Scan.Registry, Version: "test", Verify: c.Verify,
	})
	...
	if rows[0].SHA256 != "" {
		t.Fatalf("registry sha256 = %q, want empty", rows[0].SHA256)
	}
	if got := string(mustRead(t, src+".md")); strings.Contains(got, "sha256: ") {
		t.Fatalf("description carries a hash:\n%s", got)
	}
}
