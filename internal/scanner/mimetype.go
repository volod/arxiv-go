...
func Classify(head []byte, name string, opts DetectOptions) FileType {
	...
	// Detection knows some MIME types it has no canonical extension for (OLE compound documents,
	// for example, which carry SolidWorks parts as well as legacy Office files). The file's own
	// extension is what the operator sorts by, so it fills the column rather than leaving it empty.
	if ft.Type == "" {
		ft.Type = ext
	}
