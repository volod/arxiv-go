...
// Detection recognizes some container formats it has no canonical extension for, most visibly the
// OLE compound document that carries SolidWorks parts as well as legacy Office files. The row must
// still say what the file is, so file_type falls back to the name's extension.
func TestClassifyFallsBackToNameExtension(t *testing.T) {
	ole := append([]byte{0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1}, make([]byte, 64)...)
	got := Classify(ole, "fixture-part.SLDPRT", DetectOptions{})
	if got.MIME != "application/x-ole-storage" {
		t.Skipf("detection returned %q for an OLE header; fallback test needs that type", got.MIME)
	}
	if got.Type != "sldprt" {
		t.Errorf("file_type = %q, want the lower-cased name extension %q", got.Type, "sldprt")
	}
	// A name without an extension still has nothing to fall back to.
	if bare := Classify(ole, "fixture-part", DetectOptions{}); bare.Type != "" {
		t.Errorf("file_type = %q for a name without an extension, want empty", bare.Type)
	}
	// A canonical extension from detection still wins over a lying name.
	if lying := Classify(pngSignature, "fixture.txt", DetectOptions{}); lying.Type != "png" {
		t.Errorf("file_type = %q, want the detected png", lying.Type)
	}
	// An AppleDouble sidecar keeps the extension of the file it describes, which is not its own
	// type, so it stays empty.
	appleDouble := append([]byte{0x00, 0x05, 0x16, 0x07}, make([]byte, 32)...)
	if ad := Classify(appleDouble, "._fixture.CATPart", DetectOptions{}); ad.Type != "" {
		t.Errorf("AppleDouble file_type = %q, want empty", ad.Type)
	}
}
